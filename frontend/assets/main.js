  ...
  if (numSamples <= 0) {
    if (sampleSize > 0) {
      ...
      if (labelBytes.length !== inferredSamples) {
        throw new Error("Label count does not match inferred samples.");
      }
    } else {
...
function formatSnapshotDescription(snapshot) {
  ...
  if (Number.isFinite(snapshot.datasetMultiple)) {
    parts.push(`${snapshot.datasetMultiple}├ù dataset`);
  } else if (Number.isFinite(snapshot.datasetPasses)) {
    parts.push(`${formatDecimal(snapshot.datasetPasses, 2)}├ù dataset`);
  }
...
class DigitSketchPad {
  ...
  setPixels(pixels) {
    if (!pixels || typeof pixels.length !== "number") {
      throw new Error("Invalid pixel values for the drawing pad.");
    }
...
class NetworkInfoPanel {
  ...
  update(model) {
    ...
    this.summaryElement.appendChild(this.buildSummaryLine("Layers (including output)", layerSummaries.length));
  ...
  update(payload) {
    ...
    const totalMarkup =
      ...
      <div class="neuron-detail-panel__row neuron-detail-panel__row--total">
        <div><small>╬ú</small><br><strong>${this.formatValue(payload.preActivation)}</strong></div>
        <div></div>
        <div></div>
        <div></div>
      </div>
...
class NeuralVisualizer {
  ...
  describeLayer(layerIndex) {
    if (layerIndex === 0) {
      return `Input Layer (${this.mlp.architecture[layerIndex]} neurons)`;
    }
    if (layerIndex === this.mlp.architecture.length - 1) {
      return `Output Layer (${this.mlp.architecture[layerIndex]} neurons)`;
    }
    return `Hidden Layer ${layerIndex} (${this.mlp.architecture[layerIndex]} neurons)`;
  }
